...
public class SelectorJugadores implements Serializable {
	...
	public SelectorJugadores(Partido partido) {
		List<Jugador> equipoA = (List<Jugador>) partido.getEquipoA().stream().map(inscripcion -> inscripcion.getJugador()).collect(Collectors.toList());
		List<Jugador> equipoB = (List<Jugador>) partido.getEquipoB().stream().map(inscripcion -> inscripcion.getJugador()).collect(Collectors.toList());
